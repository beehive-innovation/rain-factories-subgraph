...
export function handleImplementation(event: Implementation): void {
  let factory = new ERC20BalanceTierFactory(event.address.toHex());
...
export function handleNewChild(event: NewChild): void {
  let factory = ERC20BalanceTierFactory.load(event.address.toHex());
  if(factory){
    let verify = new ERC20BalanceTier(event.params.child.toHex());

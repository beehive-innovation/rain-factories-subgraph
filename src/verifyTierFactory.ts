...
export function handleImplementation(event: Implementation): void {
  let factory = new VerifyTierFactory(event.address.toHex());
...
export function handleNewChild(event: NewChild): void {
  let factory = VerifyTierFactory.load(event.address.toHex());
  if(factory){
    let verify = new VerifyTier(event.params.child.toHex());
